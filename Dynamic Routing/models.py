...
class UCapsuleNet(nn.Module):
    def __init__(self, image_shape, in_channels):
        ...
        self.relu1 = nn.ReLU(inplace = True)
        self.primary_capsule = PrimaryCapsule(64, 2, 2)

        self.down_cap1 = ConvCapsule((14, 14), 1, 2, 1, 2, 3, 2, 1)
        self.up_cap1 = ConvTranposeCapsule((7, 7), 1, 2, 1, 2, 4, 2, 1)
        self.re_conv1 = nn.ConvTranspose2d(4, 1, kernel_size = 4, stride = 2, padding = 1)
    ...
    def forward(self, x):
        ...
        primary_capsule = self.primary_capsule(x)
        out_down1 = self.down_cap1(primary_capsule)
        out_up1 = self.up_cap1(out_down1)
        out_cat1 = torch.cat((primary_capsule, out_up1), dim = -2)
        out = out_cat1.reshape(*out_cat1.shape[:3], -1)
        out = out.permute(0, 3, 1, 2)
        out = self.re_conv1(out)
        return torch.sigmoid(out)